...
function createTestKitchenConfig(modules: Array<{id: string, width: number}>): KitchenConfig {
  return {
...
function checkForOverlaps(renderableModules: ReturnType<typeof generate>) {
  ...
  for (let i = 1; i < renderableModules.length; i++) {
    ...
    const previousRightEdge = previous.position.x + previous.dimensions.width / 2;
...
result1.forEach((module, index) => {
  ...
  console.log(`  ${module.id}: center=${module.position.x}, left=${leftEdge}, right=${rightEdge}`);
});
...
console.log('Expected: Cabinet-n at x = Σ(width_i) for all previous cabinets i');
...
result4.forEach((module, index) => {
  ...
  console.log(`    Expected right edge: ${expectedLeftEdge + module.dimensions.width}`);
...
// Summary
const totalTests = 4;
const passedTests = [overlaps1, overlaps2, overlaps3].filter(o => o.length === 0).length + (allPositionsCorrect ? 1 : 0);
...
if (passedTests === totalTests) {
  ...
  console.log('💥 Some tests FAILED! The positioning bug may not be fully resolved.');
}
