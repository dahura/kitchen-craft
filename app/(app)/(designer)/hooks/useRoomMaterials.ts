...
import { getRoomTextureSet } from "@/core/libraries/room-texture-library";
...
const createMaterial = async (
  ...
  if (!textureSet) {
    console.warn(
      `Texture set "${textureId}" not found for ${surface}, using color fallback`,
    );
...
export const useRoomMaterials = () => {
  const roomMaterials = useRoomMaterialsStore(
    (state: any) => state.currentMaterials,
  );
  ...
  useEffect(() => {
    ...
    const loadMaterials = async () => {
      ...
      try {
        const [wallsMaterial, floorMaterial, ceilingMaterial] =
          await Promise.all([
...
export const useRoomSurfaceMaterial = (
...
export const useRoomMaterialsManager = () => {
  ...
  const updateSurfaceMaterial = useRoomMaterialsStore(
    (state: any) => state.updateSurfaceMaterial,
  );
  const setSurfaceColor = useRoomMaterialsStore(
    (state: any) => state.setSurfaceColor,
  );
  const setSurfaceTexture = useRoomMaterialsStore(
    (state: any) => state.setSurfaceTexture,
  );
  const updateMaterialProperties = useRoomMaterialsStore(
    (state: any) => state.updateMaterialProperties,
  );
  const resetToDefaults = useRoomMaterialsStore(
    (state: any) => state.resetToDefaults,
  );
